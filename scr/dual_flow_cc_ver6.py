...
bound_step = 0.5
stop_needcut_wg  = -90
overloaded_total_need_cut = 29000
overloaded_total_fg_codes = 15
sub_overloaded_total_fg_codes = 9
...
def multistocks_cut(logger, finish, stocks, MATERIALPROPS, margin_df, prob_type):
    ...
    while cond == True: # have stocks and need cut #NO.1
        print("CUTTING DUAL... ")
...
logging.basicConfig(filename=f'scr/log/UATcc-ver2-{uat}.log', level=logging.INFO, 
...
with open(f'scr/jobs_by_day/job-list-cc-uat-{uat}.json', 'r') as file:
    job_list = json.load(file)

with open(f'scr/jobs_by_day/stocks-list-cc-uat-{uat}.json', 'r') as stocks_file:
    stocks_list =json.load(stocks_file)

with open(f'scr/jobs_by_day/finish-list-cc-uat-{uat}.json', 'r') as finish_file:
...
## Loop FINISH - each TASK (by CUSTOMER) in JOB - P1.0
    for finish_item in finish_list['materialprop_finish'][materialprop_set]['group']:# STANDARD GROUP - LAYER 1
        try: # Add try-catch for any error happen -> go to next job
            ...
            ## GET STANDARD-NAME 
            standard_group = list(finish_item.keys())[0] #by small-med-big
            ...
            if added_stock_ratio <= stop_stock_ratio:
                ### CUT FG CODE WITH STOCK RATIO < 10%
            ...
            if not finish:
                ...
                logger.info(f"Finished good to cut {len(finish)} stock by standard {len(stocks_by_standard)}")
                ...
                while j <= len(avalaible_coil_center)-1 and outer_coil_center_priority_cond: #NO.2
                    # Find FG with 1st Prio from list warehouse ordering as lowest to highest available stock
                    ...
                    # logger.info(f"needcut {sum_need_cut}")
                    
                    if coilcenter_finish_df.shape[0] > 0 and sum_need_cut < 0: # co FG code voi 1st la coil center da chon
                        ...
                        icc = 0 # Inner coil center priority for First group to choose
                        ...
                        while icc <= len(inner_coil_center_order)-1 and inner_coil_center_priority_cond: #NO.3
                            ...
                            for subfinish_item in sub_finish_list['subgroup']:
                                group_name = list(subfinish_item.keys())[0] #small1-2-3-med-big
                                    ...
                                    if len(filtered_stocks_by_wh) == 0:
                                        ...
                                        while icc < len(inner_coil_center_order) - 1:
                                            ...
                                            if len(stocks_by_next_wh) > 0:
                                                ...
                                                # Then add more FG with [coil_center] as 1st Prio
                                                added_finish_df = finish_df[finish_df['1st Priority']== coil_center]
                                                
                                                # Reset inner coil center order of new add finish
                                ...
                                except ContinueSubFinish: continue
                                ...
                                if len(subfinish) <= 2 and total_need_cut_by_cust_gr < 4000 :
                                    ...
                                    # Filter for the first stock with weight > 6000
                                    try:
                                ...
                                if len(partial_stocks.keys()) > 0:
                                    ...
                                    try: # START OPTIMIZATION
                                        ...
                                        # refresh only take FG with need_cut < 0
                                        ...
                                        if len(partial_stocks.keys()) == 1 and len(neg_subfinish.keys()) == 1: #### SEMI CASE
                                        ...
                                        elif len(neg_subfinish.keys()) <= 3:  #### REWIND CASE
                                            ...
                                            try:
                                                ...
                                                stocks_by_standard.pop(list(partial_stocks.keys())[0]) # truong hop ko cat duoc 
                                                stocks_by_standard.update(remained_stocks)     # thi 2 dong nay bu tru nhau
                                ...
                                moved_subfinish = move_finish(subfinish, over_cut) # co truong hop con thua moved-nhung do ko the cat duoc
                                ...
                                if not final_solution_patterns:
                                    ...
                                    filename = f"scr/results/UATresult-{uat}-job{i}-{group_name}-nosolution.xlsx"
                                    ...
                                    end_time = time.time()
                                    cleaned_materialprop_set = clean_filename(materialprop_set)
                                    filename = f"scr/results/UATresult-{uat}-job{i}-{group_name}-{coil_center}.xlsx"
                            ...
                            while icc < len(inner_coil_center_order) - 1: # NO.4
                                ...
                                if len(current_total_warehouse_stocks) == 0:
                                    ...
                                    next_inner_coil_center_priority_cond = (not hasnt_negative_over_cut and (len(next_warehouse_stocks)!=0)) #can cat tiep va co coil o next priority
                    ...
                    if j < len(avalaible_coil_center) - 1: # go from ZERO 
                        j +=1
