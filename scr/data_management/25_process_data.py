...
fin_file_path = f"scr/data/finish_uat_{uat}.xlsx"
mc_file_path = "scr/data/mother_coil_uat.xlsx"
...
def merge_standards_with_low_count(mdf):
    """_Rules_
...
for i, materialprop in enumerate(materialprops):
    print(f"MC CODE {i} {materialprop}")
    # LOAD JOB
    ...
    for coil_center in st1_coil_center_priority_list:
        ...
        merged_df = merge_standards_with_low_count(fin_df) #'cut-standard la cai duoc merge
...
with open(f'scr/jobs_by_day/finish-list-cc-uat-{uat}.json', 'w') as json_file:
    json.dump(finish_list, json_file, indent=2)
with open(f'scr/jobs_by_day/job-list-cc-uat-{uat}.json', 'w') as json_file:
    json.dump(job_list, json_file, indent=2)
with open(f'scr/jobs_by_day/stocks-list-cc-uat-{uat}.json', 'w') as stocks_file:
