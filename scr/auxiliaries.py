...
def filter_stocks_by_group_standard(finish, stocks):
    ...
    if customer_gr == "medium":
        ...
        filtered_stocks = {k: v for k, v in stocks.items() if v['weight'] >= min_mc_weight}
    elif customer_gr == "big" and len(list(finish.items())) >2:
        # Filter out stock < 7000
        filtered = {k: v for k, v in stocks.items() if v['weight'] >= max_coil_weight}
        if len(filtered) == 0:
            filtered_stocks = {k: v for k, v in stocks.items() if v['weight'] >= 4000}
        else:
...
def partite_stock(stocks, allowed_cut_weight, group, medium_mc_weight):
    ...
    if "big" in group:
        ...
        if len(filtered_stocks) < 2:
            filtered_stocks = {k: v for k, v in stocks.items() if v['weight'] >= 5000}
        else: pass
...
def check_subgroup_fg(df, sum_need_cut, standard):
    ...
    if count == 0:
        ...
        if round(count/2) >=5 : no_each_group = round(count/2)
